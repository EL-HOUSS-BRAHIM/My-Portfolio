...
class Validator
{
     ...
     */
    public static function sanitizeString(string $value, bool $strict = false): string
    {
        $clean = htmlspecialchars(trim($value), ENT_QUOTES, 'UTF-8');
        return $strict ? strip_tags($clean) : $clean;
    }
    
    public static function validateImage(array $file, object $config): array
    {
        $errors = [];
        $debug = (class_exists('Config') && \Config::getInstance()->get('app.debug')) 
            || (isset($_ENV['APP_DEBUG']) && filter_var($_ENV['APP_DEBUG'], FILTER_VALIDATE_BOOLEAN));

        // Check if file was uploaded
        if (!isset($file['tmp_name']) || empty($file['tmp_name'])) {
            $errors[] = 'No file uploaded';
            if ($debug) {
                error_log('[Validator] No file uploaded');
            }
            return $errors;
        }
        ...
        if ($imageInfo === false) {
            ...
            if ($debug) {
                error_log('[Validator] Not a valid image');
            }
        }
        ...
        if ($file['size'] > $maxSize) {
            ...
            if ($debug) {
                error_log('[Validator] File too large: ' . $file['size']);
            }
        }
        ...
        if (!in_array($mimeType, $allowedTypes)) {
            ...
            if ($debug) {
                error_log('[Validator] Invalid file type: ' . $mimeType);
            }
        }
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            if ($debug) {
                error_log('[Validator] Upload error: ' . $file['error']);
            }
        }
    ...
    public static function validateTestimonial(array $data): array
    {
        $errors = [];
        $debug = (class_exists('Config') && \Config::getInstance()->get('app.debug')) 
            || (isset($_ENV['APP_DEBUG']) && filter_var($_ENV['APP_DEBUG'], FILTER_VALIDATE_BOOLEAN));
        ...
        if (!self::required($data['name'])) {
            ...
            if ($debug) {
                error_log('[Validator] Testimonial: Name required');
            }
        } elseif (!self::maxLength($data['name'], 100)) {
            $errors['name'] = 'Name must be less than 100 characters';
            if ($debug) {
                error_log('[Validator] Testimonial: Name too long');
            }
        }
        ...
        if (!self::required($data['rating'])) {
            ...
            if ($debug) {
                error_log('[Validator] Testimonial: Rating required');
            }
        } elseif (!self::numeric($data['rating']) || !self::between($data['rating'], 1, 5)) {
            $errors['rating'] = 'Rating must be between 1 and 5';
            if ($debug) {
                error_log('[Validator] Testimonial: Rating invalid');
            }
        }
    ...
    public static function validateContact(array $data): array
    {
        ...
        if (!self::required($data['name'])) {
            $errors['name'] = 'Name is required';
            if ($debug) {
                error_log('[Validator] Contact: Name required');
            }
        } elseif (!self::maxLength($data['name'], 100)) {
            $errors['name'] = 'Name must be less than 100 characters';
            if ($debug) {
                error_log('[Validator] Contact: Name too long');
            }
        }
        ...
        if (!self::required($data['email'])) {
            ...
            if ($debug) {
                error_log('[Validator] Contact: Email required');
            }
        } elseif (!self::email($data['email'])) {
            $errors['email'] = 'Please enter a valid email address';
            if ($debug) {
                error_log('[Validator] Contact: Email invalid');
            }
        }
        ...
        if (!self::required($data['message'])) {
            ...
            if ($debug) {
                error_log('[Validator] Contact: Message required');
            }
        } elseif (!self::minLength($data['message'], 10)) {
            $errors['message'] = 'Message must be at least 10 characters long';
            if ($debug) {
                error_log('[Validator] Contact: Message too short');
            }
        } elseif (!self::maxLength($data['message'], 2000)) {
            $errors['message'] = 'Message must be less than 2000 characters';
            if ($debug) {
                error_log('[Validator] Contact: Message too long');
            }
        }
